...
class LlamaLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        # done
        attn = self.attention_norm(x)  # layer normalization
        self_attn = self.attention(attn)  # self-attention
        res_attn = x + self_attn  # residual connection
        norm_attn = self.ffn_norm(res_attn)  # layer normalization
        ffn = self.feed_forward(norm_attn)  # feed-forward network
        return res_attn + ffn  # residual connection
